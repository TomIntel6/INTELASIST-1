/*
  # Fix Admin Tables - Use usuarios.id Instead of auth.users.id
  
  This migration corrects the foreign key references to use the existing 'usuarios' table
  instead of 'auth.users', ensuring compatibility with the current system architecture.
  
  Changes:
  - Drop existing problematic tables if they exist
  - Create user_activity_log referencing usuarios.id
  - Create user_permissions referencing usuarios.id
  - Create user_permission_details
  - Update RLS policies to work with usuarios.id
*/
